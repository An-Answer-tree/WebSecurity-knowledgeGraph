...
with open('./result_of_ner.csv', newline='', encoding='utf-8') as csvfile:
    reader = csv.reader(csvfile)
    # 将数据存储到列表中
    features = list(reader)

# import relationship matrix
relationship_matrix = np.loadtxt('./0.87_0_1_matrix.csv', delimiter=',')
matrix_size = 2818

# 在Neo4j数据库中创建节点
with driver.session() as session:
    session.write_transaction(clear_all_nodes)
    session.write_transaction(create_event_nodes, title_list, features)
